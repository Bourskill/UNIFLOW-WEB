...
// --- Archivo con VARIAS piezas juntas (un archivo por talla, no por pieza) -

/**
 * Un archivo real de moldería (Illustrator/CorelDraw) trae todas las piezas
 * de una talla juntas, cada una nombrada. Esto analiza el archivo completo y
 * trata de matchear cada forma nombrada contra los roles ya definidos en el
 * Grupo (por nombre exacto, sin distinguir mayúsculas) — lo que no matchea
 * queda para que el usuario lo asigne a mano, nunca se adivina.
 *
 * @param {string} svgTexto
 * @param {string[]} roles  los roles definidos en el Grupo (ej. ["Espalda", "Manga izquierda"])
 */
export async function analizarSvgMultiple(svgTexto, roles) {
  const raiz = await parseSvg(svgTexto);
  const candidatos = [];
  recolectarCandidatos(raiz, [], candidatos);
  ...
  const asignaciones = candidatos.map((candidato, indice) => {
    const rolCoincidente = candidato.nombre
      ? roles.find((r) => r.toLowerCase() === candidato.nombre.toLowerCase())
      : null;
    return { indice, nombreDetectado: candidato.nombre, rolAsignado: rolCoincidente || null };
  });

  const rolesAsignados = new Set(asignaciones.map((a) => a.rolAsignado).filter(Boolean));
  const rolesSinAsignar = roles.filter((r) => !rolesAsignados.has(r));
...
/**
 * Segundo paso: ya con el mapeo rol -> índice de candidato confirmado (el
 * automático más las correcciones a mano del usuario), calcula la geometría
 * real de cada rol. La escala es UNA sola para todo el archivo (todas las
 * piezas de una talla salen del mismo documento) — si no viene declarada, se
 * deriva del ancho conocido de UNA pieza de referencia que el usuario elige.
 *
 * @param {string} svgTexto
 * @param {Record<string, number>} mapaRolAIndice
 */
export async function resolverGeometriasMultiples(
  svgTexto,
  mapaRolAIndice,
  ...
  const geometriasPorRol = {};
  for (const [rol, indice] of Object.entries(mapaRolAIndice)) {
    const candidato = candidatos[indice];
    if (!candidato) throw new Error('El candidato asignado a "' + rol + '" ya no existe en este SVG.');
    geometriasPorRol[rol] = poligonoYBoundingBox(candidato.pathAbsoluto, mmPorUnidadFinal, tolerancia);
  }

  return { geometriasPorRol, mmPorUnidad: mmPorUnidadFinal };
}
